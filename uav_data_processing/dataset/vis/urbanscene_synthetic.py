...
"""
UrbanScene3D Dataset using WAI format data.
"""
...
from wai.core import load_data, load_frame

class UrbanSceneSyntheticWAI(BaseDataset):
    def __init__(
        self,
        *args,
        ROOT,
    ...
    def _load_data(self):
        ...
        split_metadata_path = os.path.join(
            ...
            self.split,
            f"urbanscene_synthetic_scene_list_{self.split}.npy",
        )
    ...
    def _get_views(self, sampled_idx, num_views_to_sample, resolution, interval):
        ...
        view_indices = self._sample_view_indices_sequence(num_views_to_sample, num_views_in_scene, interval)
...
def get_parser():
    ...
    parser.add_argument(
        "-rd", "--root_dir", 
        default="/media/csuzhang/xianyan/dataset/data/urbanscene_synthetic", 
        type=str,
    )
    parser.add_argument(
        "-dmd",
        "--dataset_metadata_dir",
        default="/media/csuzhang/xianyan/dataset/metadata",
        type=str,
    )
    parser.add_argument(
        "-nv",
...
if __name__ == "__main__":
    # how to use
    # python urbanscene_synthetic.py --viz --serve

    import rerun as rr
    from tqdm import tqdm

    from base.base_dataset import view_name
    from utils.image import rgb
    from utils.viz import script_add_rerun_args
    ...
    dataset = UrbanSceneSyntheticWAI(
    ...
    if args.viz:
        rr.script_setup(args, "UrbanSceneSynthetic_Dataloader")
        rr.set_time("stable_time", sequence=0)
        rr.log("world", rr.ViewCoordinates.RDF, static=True)

    sampled_indices = np.random.choice(len(dataset), size=15, replace=False)
