...
def crop_resize_if_necessary_with_raw(
    image,
    resolution,
    depthmap=None,
    intrinsics=None,
    ...
    # High-quality Lanczos down-scaling
    raw_resolution = (resolution[0]*scale, resolution[1]*scale)
    target_rescale_resolution = np.array(raw_resolution)
    raw_image, depthmap, intrinsics, additional_quantities = (
        rescale_image_and_other_optional_info(
            image=image,
            output_resolution=target_rescale_resolution,
            depthmap=depthmap,
    ...
    if intrinsics is not None:
        new_intrinsics = camera_matrix_of_crop(
            input_camera_matrix=intrinsics,
            input_resolution=raw_image.size,
        ...
        # Create a centered crop if no intrinsics are available
        w, h = raw_image.size
        target_w, target_h = raw_resolution
        left = (w - target_w) // 2
        top = (h - target_h) // 2
        crop_bbox = (left, top, left + target_w, top + target_h)
