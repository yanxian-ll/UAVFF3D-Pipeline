"""Create train/validation/test scene lists for A3D-Bench datasets.
...
def a3d_real_split(scenes):
    train_scenes, val_scenes, test_scenes = [], [], []
    for scene in scenes:
        if scene.startswith(("nanfang_", "yanghaitang_", "xiaoxiang_")):
            test_scenes.append(scene)
        elif scene in [
            "219d41d4a83ed588c9a523c5",
            "2dcadc977905a0d50a860340",
            "3c9bff4778c7706ecf92a9cd",
            "6231e01e5574a42ec5ad0bf7",
            "347a14c5b0cda1846f3b900c",
            "602c385ca66fb22cf3a8c15d",
            "aee0a1297d3429ea49311619",
            "62550ba11757ce09b8114c78",
            "3a1a720bfdc2cedbd8d7a528",
            "64b098bc553f7dc0976ee380"
        ]:
            val_scenes.append(scene)
            train_scenes.append(scene)
        else:
...
def a3d_syn_small_split(scenes):
    train_scenes, val_scenes, test_scenes = [], [], []
    for scene in scenes:
        if scene in [
...
def a3d_syn_large_split(scenes):
    train_scenes, val_scenes, test_scenes = [], [], []
    test_focal_ambiguous_scenes = []
    ...
    for scene in scenes:
        if scene in [
            ...
            "b55e07953583c6016c5ad442",
            "7d0d879b6bf23d2e294f6370",
            "ea579d65ef9fa659433ff4ce",
            "9857fa8db1811433984c7196",
            "2014b118d54128c9237e50ed",
            "2a9a05294eb617185b2644bf",
            "2926a83104722f0172572367",
            "3d715a89eed170d7267af966",
            "f953dbbaac449dedb29b69cf",
            "cdf38df9b0510e5434c6422e"
        ]:
            val_scenes.append(scene)
            train_scenes.append(scene)
        elif scene in [
            # 不同的 FOV 和 Height，相同的场景范围
            # school
            ...
            # # school oblique
            # "ea02eaef007ee5c104ccd6e0",
            # "3faaa6ff07a249741b94f0f1",
            # "756ea332d96b9a84755fd285",
            # "80a9d505d8e4e017ec768dc7",
            # "1cde418347e87c7391b52344",
            # "9ec56ccbd13497c11ca091ae",
            # "0876727319ef3987a09ac880",
            # "68efd008248fa13e72b324c8",
            # blendedmvs 中选取的三个场景
            # 不同的 FOV 和 Height，相同的场景范围
            ...
            # # bmvs_59e864b2a9e91f2c5529325 近景
            # "7635b21e92c9431a69ad41c3",
            # "0219f5b2f1a93c1aebfe8544",
            # "2ed9dfcd5f6f0f596fe3518a",
            # "e74ba35eda2c69670377a742",
            # "887f3b226238872d3dfe50b8",
            # "65bdb53d28404e098dbfa036",
            # "44c18da094cd2a1c0db28d96",
            # "b814a8ad93de2c5027078512",	
        ]:
            test_focal_ambiguous_scenes.append(scene)
        else:
            train_scenes.append(scene)
    return train_scenes, val_scenes, test_scenes, test_focal_ambiguous_scenes
...
def process_dataset(root, metadata, dataset, scene_workers=8, file_workers=0, skip_existing_json=False):
    ...
    elif dataset == "A3D-Real":
        train, val, test = a3d_real_split(scenes)

    elif dataset == "A3D-Syn-L":
        train, val, test, test_focal = a3d_syn_large_split(scenes)
        if len(test_focal) > 0:
            save_split_outputs(
                ...
                dataset=dataset,
                split_name="test",
                scene_list=test_focal,
                scene_suffix="focal_ambiguous_scene_list",
                hfov_suffix="focal_ambiguous_scene_hfov",
                scene_workers=scene_workers,
                file_workers=file_workers,
                skip_existing_json=skip_existing_json,
            )
    elif dataset == "A3D-Syn-S":
        train, val, test = a3d_syn_small_split(scenes)
...
def main():
    ...
    parser.add_argument("--metadata", default="../data/metadata")
    parser.add_argument(
        "--dataset",
        nargs='+',
        default=["A3D-Real", "A3D-Syn-L", "A3D-Syn-S", "uavscenes", "blendedmvs", "usegeo", "whu_whuomvs", "urbanscene3d", "enrich"]
    )
    parser.add_argument(
        "--scene_workers",
        type=int,
