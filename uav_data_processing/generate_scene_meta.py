"""Generate A3D/WAI-style ``scene_meta.json`` files.

The converter expects every scene to use the common A3D layout:
...
Optional same-stem modalities such as ``mask/``, ``depth_da3/``,
``depth_complete/``, and ``semantic_mask/`` are attached only when present.
"""
...
def generate_scene_meta(data_dir: str, scene_name: str, dataset_name: str, version: str = "1.0"):
    ...
    optional_dirs = {
        ...
        "depth_da3": ("depth_da3", [".exr"], "depth"),
        "depth_complete": ("depth_complete", [".exr"], "depth"),
        "semantic_mask": ("semantic_mask", [".png"], "semantic"),
    }
    optional_files = {
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate scene_meta.json for processed A3D/WAI-format scenes.",
