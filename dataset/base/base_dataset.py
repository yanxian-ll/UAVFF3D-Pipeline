...
class BaseDataset(EasyDataset):
    ...
    def __init__(
        ...
        if transform == "imgnorm":
            ...
            if norm_t: t.append(norm_t)
            self.transform = tvf.Compose(t)

        elif transform == "colorjitter":
            t = [tvf.Lambda(_ensure_rgb), to_tensor, tvf.ColorJitter(0.5,0.5,0.5,0.1)]
            if norm_t: t.append(norm_t)
            self.transform = tvf.Compose(t)

        elif transform == "colorjitter+grayscale+gaublur":
            t = [
                tvf.Lambda(_ensure_rgb), to_tensor,
...
def is_good_type(v):
    """
    Check if a value has an acceptable data type for processing.
...
def view_name(view, batch_index=None):
    """
    Generate a string identifier for a view.
    
    Args:
        view: Dictionary containing view information.
        batch_index: Optional batch index for batched data.
        
    Returns:
        Formatted string "dataset/label/instance".
    """
    def sel(x):
