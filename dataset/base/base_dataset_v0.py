...
class BaseDataset(EasyDataset):
    ...
    def __init__(
        ...
        interval: Union[int, Tuple[int], List[int]] = 1,  # added for sequential sampling
    ):
        """
    ...
    def _sample_view_indices_sequence(
        self,
        num_views_to_sample,
        num_views_in_scene,
        interval=1,
        ...
        """
        Sample a view index sequence using interval + ping-pong sampling.

        Example:
            num_views_in_scene = 10
            interval = 2
            num_views_to_sample = 8
            base_seq = [0,2,4,6,8]
            -> [0,2,4,6,8,6,4,2]
        """

        # Interval sampling
        base_seq = np.arange(0, num_views_in_scene, interval)

        if len(base_seq) == 1:
            # Only one available view
            return np.repeat(base_seq, num_views_to_sample)

        if num_views_to_sample == len(base_seq):
            return base_seq.copy()

        # Build ping-pong sequence: [0,2,...,8,6,4,2]
        pingpong_seq = np.concatenate(
            [base_seq, base_seq[-2:0:-1]]
        )

        # Repeat until enough length
        repeats = int(np.ceil(num_views_to_sample / len(pingpong_seq)))
        full_seq = np.tile(pingpong_seq, repeats)

        return full_seq[:num_views_to_sample]
