...
from dataset.utils.misc import invalid_to_zeros
from dataset.utils.warnings import no_warnings
...
def recover_pinhole_intrinsics_from_ray_directions(
    ...
    if is_high_res or use_geometric_calculation:
        ...
        solution_x = torch.linalg.solve(ATA_x.float(), ATb_x.float()).squeeze(2).to(ATA_x.dtype)
        ...
        solution_y = torch.linalg.solve(ATA_y.float(), ATb_y.float()).squeeze(2).to(ATA_y.dtype)  # [B, 2]
...
def compute_forward_flow(ray_directions, K):
    """
    ray_directions: [B,H,W,3]
    K: [B,3,3]
    return: forward flow [B,H,W,2], mapping distorted → undistorted
    """
    ...
    # projected undistorted coords
    x_undist = fx * (dx / dz) + cx
    y_undist = fy * (dy / dz) + cy

    # pixel coordinate grid
    xs = torch.arange(W, device=device).float().view(1,1,W).expand(B,H,W)
    ys = torch.arange(H, device=device).float().view(1,H,1).expand(B,H,W)
...
def undistort_image_from_rays(image, ray_directions, K=None):
    """
    image: [B H W 3]
    ray_directions: [B,H,W,3]
    K: [B 3 3]
    """
    if K is None:
        K = recover_pinhole_intrinsics_from_ray_directions(ray_directions)
    forward_flow = compute_forward_flow(ray_directions, K) # B H W 2
    # # TODO: 可视化forward flow
    # from mapanything.utils.forward_flow_vis import visualize_flow
    # import matplotlib.pyplot as plt
    
    # vis = visualize_flow(forward_flow[0,...])     # flow: [H,W,2]
    # plt.imshow(vis)
    # plt.show()

    from mapanything.utils.resampling import rectification
    list_result = []
    for i in range(image.shape[0]):
        undistorted_image, mask = rectification(image.detach().cpu().numpy()[i], 
                                            forward_flow.permute(0,3,1,2).detach().cpu().numpy()[i])
        list_result.append(torch.tensor(undistorted_image, device=image.device))
    return torch.stack(list_result, 0)
