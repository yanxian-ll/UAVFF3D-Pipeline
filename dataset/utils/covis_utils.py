...
from dataset.wai.basic_dataset import BasicSceneframeDataset
from dataset.wai.intersection_check import (
    create_frustum_from_intrinsics,
    frustum_intersection_check,
)
from dataset.wai.m_ops import in_image, m_dot, m_project, m_unproject
from dataset.wai.ops import resize
...
_RIG_PATTERNS = [r"(?:^|[_\-.])rig(\d+)(?:$|[_\-.])"]

def parse_frame_cam_rig_from_name(name: str):
    # name: frame_name（不含扩展名也行）
    base = name
    rig_id, cam_id = 0, 0
    frame_id = base

    for p in _RIG_PATTERNS:
        m = re.search(p, frame_id, flags=re.IGNORECASE)
        if m:
            rig_id = int(m.group(1))
            frame_id = re.sub(p, "_", frame_id, flags=re.IGNORECASE).strip("_- .")
            break

    for p in _CAM_PATTERNS:
        m = re.search(p, frame_id, flags=re.IGNORECASE)
        if m:
            cam_id = int(m.group(1))
            frame_id = re.sub(p, "_", frame_id, flags=re.IGNORECASE).strip("_- .")
            break
    return frame_id, rig_id, cam_id
